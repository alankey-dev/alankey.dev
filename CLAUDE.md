...
- Quality gate (`npm test`): ESLint, Stylelint, Eleventy build,
  html-validate, axe-core, Lighthouse CI ≥ 95 mobile across all four
  categories, link-checker, `node:test` ≥ 80% line coverage on JS modules
  with branching logic.
- Per-page budgets: CSS ≤ 20 KB gzipped, JS ≤ 30 KB gzipped, webfont weight
  ≤ 200 KB on first visit, hero image ≤ 100 KB, LCP ≤ 1.5 s on Slow-4G
  mobile.
